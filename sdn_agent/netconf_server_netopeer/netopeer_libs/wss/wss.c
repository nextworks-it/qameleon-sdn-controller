 ...
 */
struct ns_pair namespace_mapping[] = {{"wss", "http://org/nextworks/qameleon/wss"}, {NULL, NULL}};
...
/**
 * @brief This callback will be run when node in path /wss:wss/wss:wss-id changes
 *
...
int callback_wss_wss_wss_wss_id(void **data, XMLDIFF_OP op, xmlNodePtr old_node, xmlNodePtr new_node, struct nc_err **error) {
	send_string_over_socket((char*)xml_node_to_string(new_node));
	printf("Wss ID modify callback: Raw content NEW is :%s\n",(char*)xml_node_to_string(new_node));
	return EXIT_SUCCESS;
}

/**
 * @brief This callback will be run when node in path /wss:wss changes
 *
 * @param[in] data	Double pointer to void. Its passed to every callback. You can share data using it.
 * @param[in] op	Observed change in path. XMLDIFF_OP type.
 * @param[in] old_node	Old configuration node. If op == XMLDIFF_ADD, it is NULL.
 ...
 */
/* !DO NOT ALTER FUNCTION SIGNATURE! */
int callback_wss_wss(void **data, XMLDIFF_OP op, xmlNodePtr old_node, xmlNodePtr new_node, struct nc_err **error) {
	send_string_over_socket((char*)xml_node_to_string(new_node));
	printf("Wss modify callback: Raw content NEW is :%s\n",(char*)xml_node_to_string(new_node));
	return EXIT_SUCCESS;
}
...
struct transapi_data_callbacks clbks =  {
	...
	.data = NULL,
	.callbacks = {
		{.path = "/wss:wss/wss:wss-id", .func = callback_wss_wss_wss_wss_id},
		{.path = "/wss:wss", .func = callback_wss_wss}
	}
};
